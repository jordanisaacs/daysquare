{
  ...
  outputs = { self, nixpkgs, utils, naersk, rust-overlay, sops-nix, mongofix, ... }:
    let
      ...
      redact_db_conn_string = "mongodb://${redact_db_user}:${redact_db_password}@${redact_db_container_name}:${builtins.toString redact_db_port}";

      redact_store_port = 8081;

      redact_tls_org = "daysquare";
      redact_store_tls_ca_path = "tls/server/cert/ca.pem";
      redact_client_tls_ca_folder = "storer-ca.pem";
        ...
        shellHook =
          let
            closeScript = pkgs.writeShellScriptBin "closescript" ''
              ...
              rm ./redact-store/config/config.yaml
              rm ./redact-client/config/config.yaml
            '';
